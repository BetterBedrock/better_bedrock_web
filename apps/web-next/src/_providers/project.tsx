...
interface ProjectContextProps {
  ...
  fetchProjectsBasicInfo: (
    ids: string[]
  ) => Promise<SimpleProjectDto[] | undefined>;
  ...
  postRating: (
    projectId: string,
    rating: number
  ) => Promise<ProjectRatingDto | undefined>;
...
export const ProjectProvider = ({ children }: ProjectProviderProps) => {
  ...
  const search = async (
    ...
    try {
      const { data } = await projectApi.projectControllerSearch(
        type,
        order,
        text,
        page
  ...
  const fetchProjectsBasicInfo = async (
    ids: string[]
  ): Promise<SimpleProjectDto[] | undefined> => {
    try {
      const { data } = await projectApi.projectControllerProjectsBasicInfo({
        ids,
      });
      return data;
    } catch (err) {
  ...
  const uploadFile = async (
    id: string,
    file: File
  ): Promise<UploadFileDto | undefined> => {
    try {
      const { data } = await projectApi.projectControllerUploadProjectFile(
        id,
        file
      );
      sendNotification({
  ...
  const submittedProjects = async (): Promise<
    SimpleProjectDto[] | undefined
  > => {
    try {
  ...
  const postRating = async (projectId: string, rating: number) => {
    try {
      const { data } = await projectApi.projectControllerRateProject(
        projectId,
        rating
      );
      sendNotification({
  ...
  const deleteRating = async (projectId: string) => {
    try {
      const { data } =
        await projectApi.projectControllerDeleteRating(projectId);
      sendNotification({
        type: "info",
        title: "Rating",
  ...
  const postComment = async (projectId: string, content: string) => {
    try {
      const { data } = await projectApi.projectControllerPostComment(
        projectId,
        { content }
      );
      sendNotification({
  ...
  const replyToComment = async (
    ...
    try {
      const { data } = await projectApi.projectControllerReplyToComment(
        projectId,
        parentId,
        {
          content,
